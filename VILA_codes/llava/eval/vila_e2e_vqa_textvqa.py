...
from eval_datasets import TextVQADataset
...
rice_cached_features_path = "/scratch/workspace/asureddy_umass_edu-llm_alignment/features-cache/textvqa.pkl" 
...
def construct_vqa_query(query_items, icl_demonstrs_list):
    ...
    for query_item,icl_demonstrs in zip(query_items, icl_demonstrs_list):
        ...
        for item in icl_demonstrs:
            query += f" <image> Question: {item['question']} Short Answer: {item['best_answer']}"
...
def get_output(item, n= 2, use_random= False, n_random=0):
    ...
    res = {
        "query": querys[0],
        "output": get_output_for_query(querys[0], im_lists[0],"llava_v1",5),
...
if __name__=="__main__":
    ...
    save_path = f"/home/asureddy_umass_edu/cs682/VILA_codes/results/textvqa/{model_name}_{n}-shot"
    if use_random:
        save_path += "_random-examples"
    if n_random:
        save_path += f"{n_random}_random-examples"
    save_path += ".json"
    print("new vqa_textvqa vila 3b")
    print(args)
    # doing for a max of 5k examples
