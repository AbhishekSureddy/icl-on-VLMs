...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Random n-shot evaluation")
    parser.add_argument("--n_few_shot", type=int, default=1, help="Number of demonstrations")
    parser.add_argument("--output_dir", type=str, default="dummy.pickle", help="Output directory")
    input_args = parser.parse_args()

    print("Number of demonstrations: ", input_args.n_few_shot)
    ...
    n_few_shot = input_args.n_few_shot
    count = 0
    ...
    test_df.drop(['demo_images'], axis=1).to_pickle(input_args.output_dir)
    test_df['predicted_label'] = test_df['raw_responses'].apply(lambda x: x[1:].split(" ")[0])
    acc = test_df[test_df.apply(lambda x: 1 if (x['predicted_label'] in x['target_label']) else 0, 1) == 1].shape[0]/len(test_df)
    print(f"Accuracy at {input_args.n_few_shot} shot: ", acc)
    test_df.drop(['demo_images'], axis=1).to_pickle(input_args.output_dir)
    print("Saved at ", input_args.output_dir)